...
import { classifyFileOp, classifyFileWrite, approve } from '../security/SecurityGateway'
...
// getSystemPath is defined above (shared by dir-load and file-search)
...
const IGNORE_FOLDERS = new Set([
  ...
  ipcMain.handle('index-folder', async (event, folderPath: string) => {
    try {
      ...
      const scanForIndexing = async (dir: string) => {
        let entries
        try {
